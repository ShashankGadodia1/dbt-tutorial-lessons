...
order_item_measures AS (
	...
	FROM {{ ref('int_thelook_ecommerce__order_items_products') }}
	GROUP BY 1)
SELECT
	od.order_id,
	...
	{{ is_weekend('od.shipped_at') }} AS order_was_shipped_on_weekend,
...
FROM {{ ref('stg_thelook_ecommerce__orders') }} AS od
LEFT JOIN order_item_measures AS om
	ON od.order_id = om.order_id
LEFT JOIN {{ ref('int_thelook_ecommerce__first_order_created') }} AS user_data
